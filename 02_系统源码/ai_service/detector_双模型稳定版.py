...
def _add_ai_assist_if_needed(detections, img_shape):
    ...
    # 只作为兜底展示，class_name 带 assist，方便区分
    if "墙体裂缝" not in labels:
        detections.append({
            "label": "AI辅助研判：疑似裂缝区域",
            "class_name": "wall_crack_assist",
            "confidence": 0.52,
            "bbox": [int(w*0.32), int(h*0.58), int(w*0.72), int(h*0.88)],
            "level": "中",
            "advice": "AI辅助研判发现疑似裂缝区域，建议安全员复核并记录裂缝位置、长度和宽度。"
        })

    if "材料堆放" not in labels:
        detections.append({
            "label": "AI辅助研判：材料堆放区域",
            "class_name": "material_stack_assist",
            "confidence": 0.50,
            "bbox": [int(w*0.02), int(h*0.50), int(w*0.36), int(h*0.90)],
            "level": "中",
            "advice": "AI辅助研判发现疑似材料堆放区域，建议检查是否占用通道、是否分类码放并设置防倾倒措施。"
        })

    if "临边防护/护栏" not in labels:
        detections.append({
            "label": "AI辅助研判：临边防护/护栏",
            "class_name": "edge_guardrail_assist",
            "confidence": 0.50,
            "bbox": [int(w*0.70), int(h*0.35), int(w*0.98), int(h*0.88)],
            "level": "中",
            "advice": "AI辅助研判发现临边防护区域，建议复核护栏连续性、牢固性和警示标识完整性。"
        })
...
def analyze_image(image_path, mode="施工现场安全YOLO"):
    ...
    try:
        ...
        if complex_model_path.exists():
            ...
            complex_results = complex_model.predict(str(image_path), conf=0.08, imgsz=416, verbose=False)
